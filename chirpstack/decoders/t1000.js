const t1000Decoder = (message) => {
    ...
    if (message.hasOwnProperty('fPort') && message.data != null){
        let device = message.deviceName;
        let fport = parseInt(message.fPort);
        let Bytes = Buffer.from(message.data, 'base64');
        let bytesString = bytes2HexString(Bytes).toLocaleUpperCase();
        ...
        let _iterator = _createForOfIteratorHelper(measurement),
            _step;
            
        try {
            for (_iterator.s(); !(_step = _iterator.n()).done;) {
                let message = _step.value;
                if (message.length === 0) {
                    continue;
                }
                let elements = [];
                let _iterator2 = _createForOfIteratorHelper(message),
                    _step2;
                try {
        ...
        let gatewayID = message.rxInfo[index].gatewayID;

        decoded.Gateway = gatewayID;
        decoded.rssi = message.rxInfo[index].rssi;
        decoded.snr = message.rxInfo[index].loRaSNR;
        decoded.dr = message.txInfo.dr;
        decoded.sf = 12 - decoded.dr;


        decoded.messages = decoded.messages[0];

        if (decoded.err == 0 && decoded.Valid == true && decoded.messages.length < 10){

            for (let i=0; i<decoded.messages.length; i++){
                if (decoded.messages[i].type == 'Battery'){
                    decoded.bat = decoded.messages[i].measurementValue;
                    decoded.date = new Date(decoded.messages[i].timestamp);
                }
                else if (decoded.messages[i].type == 'Longitude'){
                    decoded.lon = decoded.messages[i].measurementValue;
                }
                else if (decoded.messages[i].type == 'Latitude'){
                    decoded.lat = decoded.messages[i].measurementValue;
                }
            }
