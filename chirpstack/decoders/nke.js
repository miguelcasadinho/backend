              ...
              var sampleIndex = findIndexFromArgList(argList, tag)
                for (var i = 0; i < nb_sample_to_parse; i++) {
                  ...
                  if (available) {
                    ...
                    if (bi <= BR_HUFF_MAX_INDEX_TABLE) {
                      var precedingValue = out.series[sampleIndex].uncompressSamples[
              ...
              if (bi <= BR_HUFF_MAX_INDEX_TABLE) {
                if (i == 0) {
                  ...
                  if (bi > 0) {
                    ...
                    timestampCommon.push(
                      buffer.getNextSample(ST_U32, bi) + precedingTimestamp + Math.pow(2, bi) -                    1
                    )
                  } else {
            ...
            for (var i = 0; i < flag.nb_of_type_measure; i++) {
              ...
              if (compressSampleNb) {
                ...
                for (var j = 0; j < compressSampleNb; j++) {
                  var precedingRelativeTimestamp = out.series[sampleIndex].uncompressSamples[
                    out.series[sampleIndex].uncompressSamples.length - 1
                  ].data_relative_timestamp
                  var currentMeasure = {
                    data_relative_timestamp: 0,
          ...
          */
          ...
          if (port === 125){
            ...
            if (decodedBatch === false){
              ...
              //command ID
              cmdID =  bytes[1]; payload.zclheader.cmdID = decimalToHex(cmdID,2);
              //Cluster ID
              ...
              if((cmdID === 0x0a)|(cmdID === 0x8a)|(cmdID === 0x01)){
                ...
                if (cmdID === 0x8a) {
                  payload.zclheader.alarm = 1;
                }
                else {
                  payload.zclheader.alarm = 0;
                }
                //data index start
                ...
                if (  (clusterdID === 0x000f ) & (attributID === 0x0402)) {
                  stdData.label = "Index"+(payload.zclheader.endpoint+1) ;
                ...
                if (  (clusterdID === 0x000f ) & (attributID === 0x0055)) {
                  stdData.label = "State"+(payload.zclheader.endpoint+1) ;
                ...
                // configuration node power desc
                  ...
                  if ((bytes[index+2] &0x04) === 0x04) {
                    ...
                    index2=index2+2;
				          ...
				          if ((bytes[index+2] &0x02) === 0x02) {payload.data.rechargeable_battery_voltage = (bytes[index2]*256+bytes[index2+1])/1000;index2=index2+2;}
				          if ((bytes[index+2] &0x08) === 0x08) {payload.data.solar_harvesting_voltage = (bytes[index2]*256+bytes[index2+1])/1000;index2=index2+2;}
				          if ((bytes[index+2] &0x10) === 0x10) {payload.data.tic_harvesting_voltage = (bytes[index2]*256+bytes[index2+1])/1000;index2=index2+2;}
				        }
                payload.data = tab;
              }
              ...
              if(cmdID === 0x07){
                //AttributID
                attributID = bytes[6]*256 + bytes[7];payload.zclheader.attributID = decimalToHex(attributID,4);
                //status
                payload.zclheader.status = bytes[4];
                //batch
                payload.zclheader.decodedBatch = bytes[5];
              }
              ...
              if(cmdID === 0x09){
                ...
                payload.zclheader.decodedBatch = bytes[5];
                //AttributType
                payload.zclheader.attribut_type = bytes[8];
                //min
                payload.zclheader.min = {}
                if ((bytes[9] & 0x80) === 0x80) {
                  payload.zclheader.min.value = (bytes[9]-0x80)*256+bytes[10];
                  payload.zclheader.min.unity = "minutes";
                } 
                else {
                  payload.zclheader.min.value = bytes[9]*256+bytes[10];
                  payload.zclheader.min.unity = "seconds";
                }
                //max
                payload.zclheader.max = {}
                if ((bytes[9] & 0x80) === 0x80) {
                  payload.zclheader.max.value = (bytes[9]-0x80)*256+bytes[10];
                  payload.zclheader.max.unity = "minutes";
                } 
                else {
                  payload.zclheader.max.value = bytes[9]*256+bytes[10];
                  payload.zclheader.max.unity = "seconds";
                }
              }   
            }
            else{
              var brData = (brUncompress(4,[
              ...
              for (var i = 0; i < data_length; i++) {               
                if(brData["datas"][i]["data"]["label"]=="BatteryVoltage") {
                  decoded.battery = brData["datas"][i]["data"]["value"]
                } else {
              ...
              payload.data = tab;
              payload.zclheader = {};
              payload.zclheader.report = "batch";
              decoded.data = tab;
              var lst = payload.data.length;
              lst = lst -1;
              decoded.volume = payload.data[3].value;
              if (lst > 4) {
                decoded.vol_index2 = payload.data[lst].value;
              }
            }
          }
          ...
          var leitura_inicial = Number(message.tags.LEITURA_INICIAL);
          var leitura_inicial_inv = Number(message.tags.LEITURA_INICIAL_INV)
          var litros_impulso = Number(message.tags.LITROS_IMPULSO);
          var relatorio = message.tags.RELATORIO;
          ...
          if (decoded.hasOwnProperty('battery') && message.tags.hasOwnProperty('LEITURA_INICIAL_INV')){
            var payload = {
              ...
              "fPort": message.fPort,
              "Date": new Date(),
              "Volume": parseFloat((((decoded.volume*litros_impulso)*0.001)+leitura_inicial).toFixed(2)),
              "Volume_inv": parseFloat((((decoded.vol_index2*litros_impulso)*0.001)+leitura_inicial_inv).toFixed(2)),
              "Deltas": decoded.data,
              "index1": [],
              "index2": [],
          ...
          else if(decoded.hasOwnProperty('battery') && !message.tags.hasOwnProperty('LEITURA_INICIAL_INV')){
              var payload = {
                "AppID": message.applicationID,
                "Application": message.applicationName,
                "DeviceName": message.tags.CONTADOR,
          ...
          if (decoded.hasOwnProperty('data')){
            ...
            else if (relatorio == "5m_1h") {
                var rh = 12
            }
            else if (relatorio == "5m_4h") {
              var rh = 12
            }
            else if (relatorio == "15m_4h") {
              var rh = 4
            }
            else {
              var rh = 1
          ...
          for (var i = 0; i < payload.Deltas.length; i++) {
            if (payload.Deltas[i].label == 'Index1'){
              payload.index1.push({
            ...
            if (payload.Deltas[i].label == 'Index2'){
              payload.index2.push({
                "device": payload.DeviceName,
                "date": payload.Deltas[i].date,
